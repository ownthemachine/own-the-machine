# Help build Own the Machine

A working proposal for lasting citizen ownership of automation-related capital.
The project grew out of David Vanheeswijck's published book, The Horse Is Here
to Stay. The draft is not filed or registered. Its legal basis, compulsory
participation mechanism and sizing remain under review. Payments are not
promised and would depend on income actually realised by a proposed Reserve.

## Your first ten minutes

1. Read the [brief](https://ownthemachine.eu/brief) and
   [current status](https://ownthemachine.eu/about). You do not need to read
   the whole Regulation before asking a question.
2. Write to [hello@ownthemachine.eu](mailto:hello@ownthemachine.eu) with what
   you could help with, the time you could realistically offer and one question.
   If you are considering the statutory organiser role, add your EU country
   of residence. Do not send identity documents at this stage.
3. Agree a short conversation and one small first task. Participation in
   this introduction does not commit you to filing or endorsing every article.

## Choose a first task

| Contribution | A useful first task | Suggested time, not a commitment |
|---|---|---|
| Evidence | Check one cited figure against its original source | 45 minutes |
| Language | Identify one unclear paragraph in a language you know well | 30 minutes |
| Accessibility | Try the brief and contact route with your usual tools | 30 minutes |
| Administration | Test this onboarding guide and note missing information | 30 minutes |
| Law or economics | Explain one precise uncertainty and the evidence needed | 60 minutes |
| Coordination | Help prepare the next open working meeting | 45 minutes |

These are examples, not unpaid professional engagements or a promise that
anyone has been appointed. You can propose a smaller task or stop at any time.
The editor agrees the scope and checks the result before publication.

## What happens after the first task

We discuss whether you want to continue as a contributor, explore a formal
organiser role, or finish here. A contributor does not become a statutory
organiser automatically. Before any formal commitment, the prospective group
must review the registered objectives it would seek, responsibilities,
representation, data handling, costs, unresolved risks and withdrawal arrangements.
Any necessary verification should use a defined secure process, not a public
issue or this initial email exchange. No identity-document collection system
has been established by this guide.

The statutory group needs at least seven eligible EU citizens residing in at
least seven Member States. Different nationalities are not required. MEPs do
not count toward the minimum. Names of organisers are published in the official
register. The [official rules](https://citizens-initiative.europa.eu/how-it-works/faq_en)
govern eligibility, documentation and responsibilities; this guide does not
replace them. Final formal commitments require informed individual agreement.

## How we work

The aim is to automate repetitive production and administration while keeping
named people responsible for decisions, relationships and review. Roles need
not be paid jobs. Do not assume donated hours, services or financing until the
person providing them has agreed. Changes to public claims use the reviewer
pipeline. Advice and a reply are not endorsements. No one speaks for another
person or organisation without permission.

Email is used to answer your enquiry. It is not permission to add you to a
mailing list or publish your details. GitHub issues are public. See the
[privacy information](https://ownthemachine.eu/about). We ask before sharing an
introduction with another contributor. You can ask us to close your enquiry.
